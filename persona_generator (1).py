...
try:
    ...
    st.info("Please set your GOOGLE_API_KEY environment variable with your actual API key.")
    st.stop()

# --- PDF Text Extraction (few-shot examples) ---
pdf_files = [
...
def clean_prompt(text: str) -> str:
    """Remove formatting markers and newlines, keep plain text."""
    text = re.sub(r"\*\*", "", text)  # remove bold markers
    return text.replace("\n", " ").strip()

def generate_description_from_api(name: str, persona_text: str) -> str:
    """Use Gemini to generate 1–2 sentence personality description."""
    try:
...
def generate_character_persona(name, profession):
    """Call Gemini API to generate persona text."""
...
def process_character(idx, name, dob, profession, persona_text):
    """Process raw persona into structured row for Excel."""
    ...
    return {
        ...
        "native_language": "",  # keep empty
        "is_multilingual": "",  # keep empty
...
if generate_button:
    ...
    st.session_state.generated_personas_data = all_personas_data
    ...
    if all_personas_data:
        ...
        df_personas = pd.DataFrame(all_personas_data)
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
            df_personas.to_excel(writer, index=False, sheet_name="Personas")
        output.seek(0)

        st.download_button(
            label="Download Personas as Excel",
            data=output,
            file_name="character_personas.xlsx",
